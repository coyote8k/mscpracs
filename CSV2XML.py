...
[x1, X2] = [0.2, 0.6] and the weights are [w1, w2] =[0.3, 0.7]. Since the bias is included b = 0.45 and bias input xo is equal to 1, the net input is calculated as
Yin= b+x1W1 +X2W2
...
The given function gives an output only when x1 = 1 and x2 = 0. The weights have to be decided only after the analysis. The net can be represent as shown in figure below:
...
From the calculated net inputs, now it is possible to fire the neuron for input (1, 0) only by fixing a threshold of 1, i.e., θ ≥ 1 for Y unit. Thus,
w1 = 1, w2 = -1; θ ≥ 1
...
It is important to note that the neurons must be previously connected, sufficiently close to one another, so that the synapse can be reinforced. Hebb's principle can be described as a method of determining how to alter the weights between model neurons. The weight between two neurons increases if the two neurons activate simultaneously, and reduces if they activate separately. Nodes that tend to be either both positive or both negative at the same time have strong positive weights, while those that tend to be opposite have strong negative weights.
...
4.	Repeat steps 2 and 3, until the cluster centers do not change during the subsequent iterations.	5.		
...
c = 𝑎𝑟𝑔𝑖 min || x – 𝑚𝑖 ||
to find the neuron which best matches this observation. Let x denote the training vector from the observation and 𝑚𝑖 denote a single neuron in the matrix. Update that neuron to resemble that observation using the following equation:
𝑚𝑖 (𝑡 + 1) = 𝑚𝑖 (𝑡) + ℎ(𝑡)[𝑥(𝑡) − 𝑚𝑖 (𝑡)] (4) 𝑚𝑖(𝑡): 
the weight vector before the neuron is updated. 
(𝑡 + 1): the weight vector after the neuron is updated. 
(𝑡): the training vector from the observation. 
ℎ(𝑡): the neighborhood function (a smoothing kernel defined over the lattice points), defined though the following equation: 
ℎ(𝑡) = { 𝛼(𝑡), 𝑖∈𝑁𝑐 0, 𝑖∈𝑁𝑐 (5) 
 : the neighborhood set, which decreases with time. 
(𝑡): the learning-rate factor which can be linear, exponential or inversely proportional. 
...
The Hopfield model (226), consists of a network of N neurons, labeled by a lower index i, with 1≤i≤N. Similar to some earlier models (335; 304; 549), neurons in the Hopfield model have only two states. A neuron i is ‘ON’ if its state variable takes the value Si=+1 and ‘OFF’ (silent) if Si=-1. The dynamics evolves in discrete time with time steps Δt. There is no refractoriness and the duration of a time step is typically not specified. If we take Δt=1ms, we can interpret Si(t)=+1 as an action potential of neuron i at time t. If we take Δt=500ms, Si(t)=+1 should rather be interpreted as an episode of high firing rate.
Neurons interact with each other with weights wij. The input potential of neuron i, influenced by the activity of other neurons is
	hi(t)=∑jwijSj(t).		(17.2)
The input potential at time t influences the probabilistic update of the state variable Si in the next time step:
	Prob{Si(t+Δt)=+1|hi(t)}=g(hi(t))=g(∑jwijSj(t))		(17.3)
where g is a monotonically increasing gain function with values between zero and one. A common choice is g(h)=0.5[1+tanh(βh)] with a parameter β. For β→∞, we have g(h)=1 for h>0 and zero otherwise. The dynamics are therefore deterministic and summarized by the update rule
